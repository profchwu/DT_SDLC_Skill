...
`sdlc` 新增持續完成已授權工作的條件：只要仍有安全且可執行的工作，就繼續實作與驗證，不把分批成果、測試綠燈或待辦清單當成停止理由。內部階段關卡以進度更新呈現；只有事先約定的審查點、重大決策或新授權才要求使用者確認。遇到阻礙時，先完成其他未受阻擋的工作，再說明具體阻礙與所需協助；不得降低驗收標準或宣稱未驗證的工作完成。使用者暫停、授權邊界及實際平台限制仍須遵守。
